...
def app():
    ...
    Entry_price = int(used_car[(used_car['Maker'] == Maker) & (used_car['Genmodel'] == Genmodel) & (used_car['Year'] == Year)]['Entry_price'].head(1))
    price = int(used_car[(used_car['Maker'] == Maker) & (used_car['Genmodel'] == Genmodel) & (used_car['Year'] == Year)]['Price'].head(1))
    ...
    st.write(f'Difference in Won: {int((prediction_to_won-price_to_won) / 10000)} 만원')
