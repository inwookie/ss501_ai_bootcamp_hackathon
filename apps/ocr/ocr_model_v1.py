...
def find_numplate(img):#pathed
    ...
    #print("plate_infos", plate_infos[longest_idx])
    #st.image(plate_img, use_column_width=True)
    #print("plate_infos", plate_infos)
    #print("info", info)
    img_out = img.copy()

    
    crop_img = img_out[info['y']: info['y']+info['h'], info['x']: info['x']+info['w']]
    st.write("""
    ...
    st.write(chars)
    cv2.rectangle(img_out, pt1=(info['x'], info['y']), pt2=(info['x']+info['w'], info['y']+info['h']), color=(255,255,255),thickness=-1)
    st.write("""
